...
// ROI(注目帯)。値を小さくする=画面の上=遠く(先)を見る。高速で曲がりきるには手前だけでなく
// 約1車身先のカーブを早く捉えて反応時間を稼ぐ(先読み)。上げ過ぎると近接追従が甘くなる。
const float ROI_TOP_R    = 0.48f;  // ROI の上端 (画像高さ比, 小さいほど遠く=先読み)
const float ROI_BOT_R    = 0.80f;  // ROI の下端 (画像高さ比)
