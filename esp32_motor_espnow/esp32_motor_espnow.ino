...
// 0=減速なし。0.8なら最大舵角時に base を (1-0.8)=20% まで落とす。直線では据え置き。
// 高速(BPM180+)でコーナーを曲がりきれず脱線する問題への対策。下げ過ぎると失速。
const float CORNER_SLOWDOWN = 0.8f;
// コーナー絶対速度上限(%)。比例減速だけだと誤差が最大化するまで十分減速せず反応が遅れる
// ため、コーナーと判定した時点で base を即この上限に頭打ちにする。240BPM等で直線(g_baseSpeed)
// をいくら速くしても、カーブはこの速度を超えない=曲がりきれず脱線するのを防ぐ。直線では効かない。
const int   CORNER_MAX_SPEED = 38;
// |正規化誤差|がこれを超えたら「コーナー」とみなし CORNER_MAX_SPEED の上限を適用する。
...
LineResult lineTrace() {
  ...
  // コーナーと判定したら絶対速度上限で頭打ちにする。比例減速はaerrが最大化するまで十分
  // 減速せず反応が遅れる(高速で曲がりきれず脱線)ため、コーナー検知で即上限へ前倒しする。
  if (aerr > CORNER_CAP_AERR && base > CORNER_MAX_SPEED) base = CORNER_MAX_SPEED;
