...
//  ※しきい値はローカル適応式(各画素を周囲の局所平均±マージンと比較)で、影や外光の
//    明るさムラに強い。較正(c)時は実測した明暗方向と床/線の輝度差をマージンに使う。
...
const int   LINE_MARGIN  = 35;     // 各画素を「周囲の局所平均」と比べ、これだけ離れていれば線とみなす(未較正時)
const float LOCAL_BG_R   = 0.22f;  // 局所背景ウィンドウの半幅(画像幅比)。線幅より十分広く取る
const int   MIN_LINE_PIX = 8;      // ROI内でこの数未満なら「ライン消失」
// ROI(注目帯)。先読みは「帯を上にずらす」のではなく「帯を広げて手前+先の両方を見る」ことで行う。
// 下端(BOT)は手前=急カーブで線が最初に来る場所なので画面下に残す(ここを捨てると急カーブで
// 線がROIから外れ px<MIN_LINE_PIX で消失停止する)。上端(TOP)を上げて先のカーブも早く捉える。
const float ROI_TOP_R    = 0.50f;  // ROI の上端 (画像高さ比, 小さいほど遠く=先読みを足す)
const float ROI_BOT_R    = 0.95f;  // ROI の下端 (画像高さ比, 手前=急カーブ保持に必須)
...
// 0=減速なし。0.5なら最大舵角時に base を (1-0.5)=50% まで落とす。直線では据え置き。
// 速さの主制御は下の上限/下限(CORNER_MAX/MIN_SPEED)で行い、これは滑らかな繋ぎ役。
const float CORNER_SLOWDOWN = 0.5f;
// コーナー時の速度を上限〜下限で挟む(コーナーと判定したときだけ。直線では効かない)。
// 上限 CORNER_MAX_SPEED: 速すぎて曲がりきれず脱線するのを防ぐ。240BPM等で直線(g_baseSpeed)を
//   いくら速くしても、カーブはこの速度を超えない。比例減速より早く効かせて反応遅れを防ぐ。
// 下限 CORNER_MIN_SPEED: 減速しすぎて失速・急ピボットでライン消失して止まるのを防ぐ最低速度。
//   モーターが確実に回り、車体が前進しながら曲がれる速度にする(下げ過ぎると急カーブで停止)。
const int   CORNER_MAX_SPEED = 38;
const int   CORNER_MIN_SPEED = 24;
// |正規化誤差|がこれを超えたら「コーナー」とみなし上記の上限/下限を適用する。
// 小さくすると緩いカーブでも早めに減速(安全側)、大きくすると急カーブだけ減速(速さ優先)。
const float CORNER_CAP_AERR  = 0.25f;
...
// ---- キャリブレーション ----------------------------------------------------
// 'c' で床と線の輝度を実測し、線の明暗(暗線/明線)と床・線の輝度差を NVS に保存する。
// 検出は較正の有無に関わらず「各画素 vs 周囲の局所平均」のローカル適応式(影・外光に強い)。
// 較正済みなら明暗方向と実測輝度差(の半分)をマージンに使い、未較正なら LINE_MARGIN を使う。
...
LineResult lineTrace() {
  ...
  //    各画素を画面全体の1つのしきい値ではなく「横方向に近い周囲の局所平均」と比べる。
  //    影や外光による緩やかな明るさの傾きは局所平均ごと差し引かれて消え、周囲より
  //    局所的に暗い(明るい)線だけが残る。較正済みなら線の明暗方向と、実測した床/線の
  //    輝度差(の半分)をマージンに使う(絶対しきい値g_threshAbsは外光に弱いので局所平均
  //    比較に置換。g_threshAbsは参考表示用に残す)。局所平均はrunning sumで行ごとO(W)。
  bool lineIsDark = g_calibrated ? g_calLineIsDark : LINE_IS_DARK;
  int  margin     = g_calibrated ? max(CALIB_MIN_SEP / 2, abs(g_calFloorLv - g_calLineLv) / 2)
                                 : LINE_MARGIN;
  ...
  for (int y = rTop; y < rBot; y++) {
    const uint8_t* row = fb->buf + (long)y * W;
    // x=0 のウィンドウ [0, HW] を初期化し、x増加に合わせ右端追加・左端除去でスライド
    long wsum = 0;
    int  wn   = 0;
    ...
    for (int x = 0; x < W; x++) {
      ...
      int thresh = lineIsDark ? (localMean - margin) : (localMean + margin);
  ...
  float aerr = fabsf(norm);                     // 0..1 操舵用の曲率指標(平滑化済み)
  // 速度判定用は生の誤差も併用する。操舵をローパス(NORM_FILT_A)で平滑化している分、aerrは
  // コーナー入口で数フレーム遅れる。240BPM等の高速ではその遅れの間に全速で突っ込み、線が
  // 画面外に出て消失停止する。生の|rawNorm|でも即減速させ、入口の突っ込みを防ぐ。
  float aerrSpeed = fmaxf(aerr, fabsf(rawNorm));
  float dNorm = norm - g_prevNorm;              // 誤差の変化(微分項)。蛇行を制動する
  g_prevNorm = norm;
  int   turn = (int)((KP * norm + KD * dNorm) * 100.0f);
  turn = constrain(turn, -MAX_TURN, MAX_TURN);

  // コーナー自動減速: 急カーブ(aerrSpeed大)ほど基準速度を一時的に落として曲がりやすくする。
  // 直線(aerrSpeed≈0)では g_baseSpeed 据え置きなので直線速度は犠牲にしない。
  int base = (int)(g_baseSpeed * (1.0f - CORNER_SLOWDOWN * aerrSpeed));
  // コーナーと判定したら速度を上限〜下限で挟む。上限=速すぎ脱線を防止(検知で即前倒し)、
  // 下限=減速しすぎの失速・急ピボットによるライン消失停止を防止。
  if (aerrSpeed > CORNER_CAP_AERR) {
    if (base > CORNER_MAX_SPEED) base = CORNER_MAX_SPEED;
    if (base < CORNER_MIN_SPEED) base = CORNER_MIN_SPEED;
  }
