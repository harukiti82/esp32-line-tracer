...
// D0(GPIO1)側の接点が接触不良で左が弱っていたため、配線をD0↔D1で入れ替えて復旧した。
// 実機で動く配線を正とし、ピン定数側を入れ替える(左右の較正は servoL/servoR に紐づくので整合は保たれる)。
const int MOTOR_L_PIN = 2;                // 左モーター(ESC)信号ピン (D1)
const int MOTOR_R_PIN = 1;                // 右モーター(ESC)信号ピン (D0)
...
// 車体の傾き(ヘディング)ゲイン。ROIを手前と先の2つに分け、その重心の差=画像内の線の傾き
// (=線に対する車体の角度)を操舵に足す。横位置が中央でも車体が斜め(カーブ直後など)だと
// 線が傾いて写るので、それを検知してズレる前に自己整列させ、直線のふらつきを抑える。
// 0=無効(従来の横位置のみ)。大きすぎると直線で過敏になり震えるので小さめから現場調整する。
const float K_ANGLE = 0.4f;
...
float g_angleFilt = 0.0f;                   // ローパス後の傾き(ヘディング)項(フィルタ状態を保持)
...
LineResult lineTrace() {
  ...
  // 2) しきい値を超える(=線)画素の重心Xを求める。ROIを手前(画面下)と先(画面上)の2帯に
  //    分けて別々に重心を出す(線の傾き=車体の傾きを測り、直線のふらつきを抑えるため)。
  const int mid = (rTop + rBot) / 2;            // 手前/先の境界行
  long sxNear = 0, sxFar = 0;
  int  pxNear = 0, pxFar = 0;
  for (int y = rTop; y < rBot; y++) {
    const uint8_t* row = fb->buf + (long)y * W;
    bool isFar = (y < mid);                     // 画面上(y小)=先(遠く)
    for (int x = 0; x < W; x++) {
      bool isLine = lineIsDark ? (row[x] < thresh) : (row[x] > thresh);
      if (isLine) {
        if (isFar) { sxFar += x; pxFar++; }
        else       { sxNear += x; pxNear++; }
      }
    }
  }
  esp_camera_fb_return(fb);

  long sx = sxNear + sxFar;
  int  px = pxNear + pxFar;
  r.pixels = px;
  if (px < MIN_LINE_PIX) {        // ライン消失
    ...
    g_angleFilt = 0.0f;           // 傾き項のフィルタもリセット
    return r;
  }
  ...
  // 傾き(ヘディング)項: 先の重心と手前の重心の差=画像内の線の傾き。横位置が中央でも車体が
  // 斜め(カーブ直後など)だと線が傾いて写る。これを操舵に足すと、ズレてから直すのでなく傾いた
  // 時点で自己整列でき、直線のふらつきが減る。両帯に十分な画素があるときだけ算出する。
  float angleNorm = 0.0f;
  if (pxNear >= MIN_LINE_PIX && pxFar >= MIN_LINE_PIX) {
    int cNear = (int)(sxNear / pxNear);
    int cFar  = (int)(sxFar  / pxFar);
    angleNorm = (float)(cFar - cNear) / (W / 2.0f);   // -1..1(>0:先が右=車体が左向き)
  }
  g_angleFilt = NORM_FILT_A * angleNorm + (1.0f - NORM_FILT_A) * g_angleFilt;

  int   turn = (int)((KP * norm + KD * dNorm + K_ANGLE * g_angleFilt) * 100.0f);
