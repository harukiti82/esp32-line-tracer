...
// 100%指令時の中立からの振り幅(us)。ESC/連続回転サーボは中立から±250us程度で
// 最大回転に達し、それ以上パルスを離しても速くならない(REMOTEで50%以降頭打ち
// だったのはこれが原因。500だと上半分が不感帯になる)。実機の飽和点に合わせ、
// 0〜100%が線形に最高速まで上がるよう250に設定する。
