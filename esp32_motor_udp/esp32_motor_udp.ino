...
// 100%指令時の中立からの振り幅(us)。ESC/連続回転サーボの前進フルは中立+500us
// (=2000us)まで使える。まだ上に余力があるとの実測から、ここをフルレンジまで開け、
// 100%が2000usに到達するよう500に設定する。0〜100%は不感帯補正の最小点から
// 2000usまで線形に割り当てる(低速の出だしは DEADBAND_*_US で維持)。
//   ・100%手前で速度が頭打ち(フラット)になるなら、その%×500まで減らす
//     例: 90%以降フラットなら 450 へ
//   ・もっと回したいが2000usで頭打ちなら、ESC側の上限(SERVO_MAX_US)を要検討
// ※この値はLINEモードの%→us変換にも効くので、LINEが速すぎたら BASE_SPEED を下げる。
const int SPEED_RANGE  = 500;
